...
def get_notices_from_neighbours(user_id: int, db: Session = Depends(get_db)):
    ...
    notices = (
        db.query(NoticeDao)
        .filter(NoticeDao.street_id == street_id)
        .order_by(NoticeDao.created_at.desc())
        .all()
    )
    return notices


@router.get("/status/notice/{notice_id}", response_model=NoticeReactionStatusResponse)
def get_count_of_reactions_of_notice(notice_id: int, db: Session = Depends(get_db)):
    ...
    reaction_counts = (
        db.query(ReactionDao.reaction, func.count(ReactionDao.user_id))
        .filter(ReactionDao.notice_id == notice_id)
        .group_by(ReactionDao.reaction)
        .order_by(ReactionDao.reaction.asc())
        .all()
    )
...
@router.post(
    "/user/{user_id}/notice/{notice_id}",
    response_model=ReactionResponse,
    status_code=status.HTTP_201_CREATED,
)
def create_reactions_of_notices(
    reaction: ReactionCreateRequest, db: Session = Depends(get_db)
):
    ...
    notice_from_db = (
        db.query(NoticeDao).filter(NoticeDao.id == reaction.notice_id).first()
    )
    ...
    combo_user_notice = (
        ...
        .filter(
            ReactionDao.user_id == reaction.user_id,
            ReactionDao.notice_id == reaction.notice_id,
        )
        .first()
    )
